...
mod tests {
    ...
    fn calculates_delay_within_bounds() {
        ...
        for _ in 0..100 {
            ...
            assert!(delay.as_millis() <= 100, "First retry delay should be <= base delay");

            let delay = retry_delay.calculate_delay(1);
            assert!(delay.as_millis() <= 200, "Second retry delay should be <= 2 * base delay");

            let delay = retry_delay.calculate_delay(3);
            assert!(delay.as_millis() <= 800, "Fourth retry delay should be <= 8 * base delay");

            // Test max delay cap
            let delay = retry_delay.calculate_delay(5);
            assert!(delay.as_millis() <= 1000, "Delay should be capped at max_delay");
        }
    }
    ...
    fn applies_jitter() {
        ...
        assert!(delays.iter().all(|&d| d <= 200), "All delays should be <= 2 * base delay");
    }
    ...
    fn respects_max_delay() {
        ...
        for _ in 0..100 {
            ...
            assert!(delay.as_millis() <= 500, "Delay should respect max_delay cap");
        }
    }
}